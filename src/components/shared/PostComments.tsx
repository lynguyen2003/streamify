...
import { ADD_COMMENT, LIKE_COMMENT, DELETE_COMMENT, REPLY_COMMENT } from '@/graphql/mutations';
...
const PostComments = ({ post, onCommentSubmit }: PostCommentsProps) => {
  ...
  useEffect(() => {
    ...
    socket.on('new-comment', (newComment: IComment) => {
    ...
    socket.on('comment-liked', ({ commentId, userId }: { commentId: string, userId: string }) => {
      setComments(prevComments => 
        prevComments.map(comment => {
    ...
    socket.on('comment-deleted', (commentId: string) => {
      // Remove comment if it's a top-level comment
      setComments(prevComments => 
        prevComments.filter(comment => comment._id !== commentId)
          // Also check for replies that might be deleted
          .map(comment => {
            if (comment.replies) {
              return {
                ...comment,
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.addComment) {
        // The socket will handle adding the comment to the UI
        setComment('');
